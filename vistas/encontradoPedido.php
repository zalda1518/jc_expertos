...
<div id="main-encontrado">
  ...
  <table border="solid 1px" id="table-encontrado">
    ...
    if (empty($_POST['buscar-pedido'])) {
      echo "<script>alert('Ingrese el numero de pedido')</script>";
      echo "<script>windows.location.href='buscarPorPedido.php'</script>";
      die;
    }
    ...
    if (isset($_POST['buscar-pedido'])) {
      ...
      if (empty($dat)) {
        ...
        echo "<script>alert('Pedido no encontrada')</script>";
        echo "<script>windows.location.href='buscarPorPedido.php'</script>";
        die;
      } ?>
